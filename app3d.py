...
<div class="xs-hero">
  ...
  <span class="xs-pill">Time-Travel Order Journey</span>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    with st.expander("💰 พารามิเตอร์ Dealer", expanded=True):
        ...
        local_premium = st.number_input("Local Premium/Discount ฝั่งไทย (%)", value=0.1, step=0.1, key="bt_local_premium") / 100

    with st.expander("💳 ค่าธรรมเนียมกระดานไทย"):
        include_trading_fee_revenue = st.checkbox("รวมรายได้ค่าธรรมเนียมซื้อขาย 0.25%", value=True, key="bt_include_fee_rev")
        withdrawal_fee_markup_pct = st.slider("Markup ค่าธรรมเนียมถอน (%)", 0, 200, 0, key="bt_wd_markup") / 100
        settlements_per_day = st.number_input("รอบถอนเหรียญให้ลูกค้า/วัน", value=1, min_value=1, step=1, key="bt_settlements")
    ...
    with st.expander("🏦 สิทธิพิเศษ KTB", expanded=(bank_type.startswith("KTB"))):
        ...
        ktb_fx_spread_bps = st.number_input("ส่วนต่างเรทที่ดีกว่าตลาด (bps)", value=15.0, step=1.0, min_value=0.0, key="bt_ktb_fx_bps") if use_ktb_fx else 0.0
        ktb_wd_fee_thb = st.number_input("ค่าธรรมเนียมถอนบาท KTB (บาท)", value=15.0, step=1.0, min_value=0.0, key="bt_ktb_wd_fee")
    ...
    if asset in STABLECOINS:
        ...
        with st.expander("📉 Execution Model", expanded=True):
            slippage_sensitivity = st.number_input("Slippage Sensitivity (% ของ Volatility รายวัน)", value=10.0, step=1.0, key="bt_slip") / 100
    ...
    with st.expander("📥 Flow Assumptions", expanded=False):
        monthly_volume_thb = comma_number_input("ปริมาณธุรกรรมลูกค้าต่อเดือน (THB)", value=300_000_000, min_value=0, key="cp_volume")
        net_bias_pct = st.slider("Net Flow Bias — ลูกค้าซื้อสุทธิ(+) / ขายสุทธิ(-)", -100, 100, 20, key="cp_bias") / 100
        flow_cv_pct = st.slider("ความผันผวนของปริมาณต่อวัน (CV, %)", 10, 150, 40, key="cp_cv") / 100
        settlement_days = st.number_input("Settlement / Hedge Lag (วัน)", value=2, min_value=1, max_value=10, step=1, key="cp_lag")
        confidence = st.select_slider("Confidence Level ของ Safety Stock", options=[90, 95, 99, 99.9], value=99, key="cp_conf")
        z_alpha = Z_SCORE_MAP[confidence]

    with st.expander("💼 Capital Pool", expanded=False):
        total_capital_thb = comma_number_input("เงินทุนสภาพคล่องรวม (THB)", value=300_000_000, min_value=0, key="cp_total_capital")
        cex_margin_thb = comma_number_input("เงินทุนบนกระดานโลก / CEX Margin (THB)", value=60_000_000, min_value=0, key="cp_cex_margin")
        liab_thb = comma_number_input("หนี้สินต่อลูกค้า (THB)", value=250_000_000, min_value=1, key="cp_liab")
        cex_margin_asset = st.selectbox("สินทรัพย์ Margin บนกระดานโลก", ["Stablecoin", "เหรียญเดียวกับที่เทรด"], key="cp_cex_asset")
        cex_counterparty_haircut = st.number_input("Counterparty Haircut (%)", value=2.0, step=0.5, min_value=0.0, key="cp_cex_hc") / 100
    ...
    with st.expander("⚖️ เกณฑ์เงินกองทุน ก.ล.ต.", expanded=False):
        ...
        trading_risk_rate = st.number_input("อัตรา NC ความเสี่ยงบริการซื้อขาย (%)", value=2.0, step=0.1, min_value=0.0, key="cp_trading_risk_rate") / 100
        cold_foreign_rate = st.number_input("อัตรา NC cold wallet ต่างประเทศ (%)", value=2.0, step=0.5, min_value=1.0, key="cp_cold_foreign_rate") / 100
        hot_wallet_pct = st.slider("สัดส่วนสต็อกเหรียญที่เก็บใน Hot Wallet (%)", 0, 100, 50, key="cp_hot_pct") / 100
        cold_domestic_split_pct = st.slider("สัดส่วน Cold Wallet ที่ฝากในประเทศ (%)", 0, 100, 100, key="cp_cold_domestic_pct") / 100
...
def _sim_defaults(asset_name, start_date_val, spot_usd, usdthb, target_stock_thb):
...
def _sim_config_signature(ctx, target_stock_thb, start_date, end_date):
    keys = [
...
def _sim_normalize_state(sim, asset, start_date_val, spot_usd, usdthb, target_stock_thb):
    if not isinstance(sim, dict):
        return _sim_defaults(asset, start_date_val, spot_usd, usdthb, target_stock_thb)
...
def execute_order(sim, side, amount_thb, order_date, px_row, ctx):
    ...
    hedge_required_thb = hedge_required_coins * coin_price_global
    hedged_coins = 0.0
    residual_unhedged_coins = 0.0
    ...
    if side == "buy":
        ...
        residual_unhedged_coins = max(0.0, hedge_required_coins - hedged_coins)
        cex_used_thb_this_order = hedged_coins * coin_price_global
    ...
    steps.append(dict(
        ...
        rows=[
            ...
            ("มูลค่า hedge", f"{fmt_baht(hedge_thb)}"),
...
ทดลองใส่ออเดอร์ด้านซ้าย หรือรันอัตโนมัติ ระบบจะ**สุ่มเดินหน้าวันเวลาไปเรื่อยๆ ตามกรอบเวลาที่เลือก** เพื่อทดสอบว่าถ้ารับลูกค้าต่อเนื่องจนโควตาต่างๆ ถูกใช้ไป ด่านไหนจะแตกก่อนกัน
    ...
    if data.empty:
        ...
        if rp_sim is None:
            ...
            # ---------- STATE ----------
            sim_signature = _sim_config_signature(ctx, target_stock_thb, start_date, end_date)
            need_reset = (
            ...
            if need_reset:
                start_date_val = data.index[0]
                spot_initial = data.loc[start_date_val, "Global_USD"]
                fx_initial = data.loc[start_date_val, "USDTHB"]
                st.session_state.sim = _sim_defaults(asset, start_date_val, spot_initial, fx_initial, target_stock_thb)
                st.session_state.sim_signature = sim_signature
                st.session_state.sim_steps = []

            # Get current dynamic price from the simulator's current date
            current_date_val = st.session_state.sim["current_date"]
            # Fallback in case current_date is somehow not in index
            if current_date_val not in data.index:
                current_date_val = data.index[0]
            ...
            sim = _sim_normalize_state(st.session_state.sim, asset, current_date_val, spot_usd_current, usdthb_current, target_stock_thb)
            ...
            metric_card(
                s1[1], "โควตา Outbound FX ที่ใช้", f"$ {sim['fx_used_usd']:,.0f}",
                fx_left,
            ...
            with left:
                ...
                if send:
                    ...
                    # เลื่อนวันที่เดินหน้าไปแบบสุ่มสำหรับออเดอร์แมนนวลครั้งต่อไป
                    valid_dates = data[data.index >= current_date_val].index
                    if len(valid_dates) > 1:
                        # สุ่มวันใหม่ในอนาคต (อาจจะเป็นวันเดิมหรือวันถัดๆ ไป)
                        sim["current_date"] = np.random.choice(valid_dates)
                    else:
                        sim["current_date"] = data.index[-1]
                ...
                if run_day:
                    ...
                    if len(valid_dates) > 0:
                        chosen_dates = sorted(rng.choice(valid_dates, size=int(n_orders), replace=True))
                    else:
                    ...
                    sim["current_date"] = chosen_dates[-1]
            ...
            if sim["orders"]:
                ...
                metric_card(
                    lc[0], "จำนวนออเดอร์", f"{len(led):,}", None,
                    f"ซื้อ {buys} · ขาย {len(led)-buys}",
                )
                metric_card(
                    lc[1], "Notional รวม", fmt_baht(led["มูลค่า (บาท)"].sum()), None,
                ...
                metric_card(
                    lc[2], "มาร์จิ้นเฉลี่ย", f"{margin_bps:,.1f} bps",
                    led["กำไรออเดอร์"].sum(),
                    "P&L รวม ÷ Notional รวม",
                )
                metric_card(
                    lc[3], "ออเดอร์ที่ต้องเฝ้าระวัง/ติดด่าน", f"{blocked:,}",
                    -1 if blocked else 0,
                ...
                with g1:
                    ...
                    fig_pnl.add_trace(go.Scatter(
                        y=led["กำไรออเดอร์"].cumsum(), x=led.index,
                ...
                with g2:
                    ...
                    fig_gate.add_trace(go.Scatter(
                        y=led["FX ใช้สะสม (USD)"], x=led.index,
                        name="FX ใช้สะสม", line=dict(color="#3B82F6", width=2),
                    ))
                    fig_gate.add_hline(
                        y=fx_limit_max, line=dict(color="#FF4B4B", dash="dash"),
                ...
                fig_cex.add_trace(go.Scatter(
                    ...
                    name="CEX Liquidity ใช้สะสม",
                    line=dict(color="#8B5CF6", width=2),
                ))
                fig_cex.add_hline(
                    y=cex_liquidity_thb, line=dict(color="#FF4B4B", dash="dash"),
                ...
                fig_nc.add_hline(
                    y=0, line=dict(color="#FF4B4B", dash="dash"),
                ...
                st.download_button(
                    "⬇️ ดาวน์โหลดสมุดออเดอร์ (CSV)",
                    to_csv_bytes(led), f"xspring_orders_{asset}.csv", "text/csv"
                )
